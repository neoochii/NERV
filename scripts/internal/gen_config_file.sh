...
{
    local PLATFORM_KEY_SHA1="2a3c0cffcd174ec4a6e9039856f40cd2a836f303"

    local USES_OFFICIAL_CERT="false"
    [[ "$(sha1sum "$SRC_DIR/security/platform.pk8" 2> /dev/null | cut -d " " -f 1)" == "$PLATFORM_KEY_SHA1" ]] && USES_OFFICIAL_CERT="true"
...
#       - "config_enableDisplayCutoutProtection" in "res/values/bools.xml" inside `SystemUI.apk` is set to "true"
{
    ...
    GET_BUILD_VAR "TARGET_DISABLE_AVB_SIGNING" "true"
